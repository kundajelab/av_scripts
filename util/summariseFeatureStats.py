...
def summariseFeatureStats(options):
    ...
    outputFile = fp.getFileNameParts(options.inputFile).getFilePathWithTransformation(lambda x: "summaryStats_"+str(x), extension=".txt");
    ...
    def action(inp, lineNumber):
        if (lineNumber == 1):
            ...
            countsArr = np.array([int(x) for x in inp[1:]]);
