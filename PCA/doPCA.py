...
if __name__ == "__main__":
    ...
    parser.add_argument("--coocMatrixFile", required=True);
    parser.add_argument("--numberOfPcsToTruncateTo", type=int, required=True);
    options = parser.parse_args();

    pcaOutputFile = fp.getFileNameParts(options.coocMatrixFile).getFilePathWithTransformation(lambda x: "pca_"+x, extension=".txt");
    util.executeAsSystemCall("doPCA.R "+options.coocMatrixFile+" "+pcaOutputFile);
